...
import {
  MyGameDivPicturesProps,
  My_Type_ClassNames,
  My_Type_UseReducer_GameDivPictures_State,
  My_Type_DivImg,
  My_Type_UseReducer_GameDivPictures_Action,
} from "../../_inc/my_types";

import { useImgContext } from "../../context/ImgContext";

const reducerImg = (
  stateImg: My_Type_UseReducer_GameDivPictures_State,
  action: My_Type_UseReducer_GameDivPictures_Action,
) => {
  switch (action.type) {
...
export const GameDivPictures = ({
  ...
  useEffect(() => {
    const fetchDivItemsWithCount = async () => {
      try {
        const imgDivs: My_Type_DivImg[] = await fetchImageDivsForCounts(
          selectedImgCount,
          imgNames,
  ...
  useEffect(() => {
    setTimeout(function () {
      let selectedArr: My_Type_DivImg[] = stateImg.divImgs.filter((oneDiv) =>
        oneDiv.classNames.includes("selected_Div_img"),
      );
