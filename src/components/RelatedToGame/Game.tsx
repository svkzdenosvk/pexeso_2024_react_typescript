...
import {
  ...
  My_Type_ImgCount,
  Encrypted,
  My_Type_UseReducer_Game_State,
  My_Type_UseReducer_Game_Action,
} from "../../_inc/my_types";
import {
...
const reducer = (
  state: My_Type_UseReducer_Game_State,
  action: My_Type_UseReducer_Game_Action,
) => {
  switch (action.type) {
    case "SET_START_GAME":
      return {
...
const Game = () => {
 ...
 /*--------------------------------------------------------------------------------------------------------------------------------------------*/
  ...
  useEffect(() => {
    ...
    try {
      decryptedSettings = simpleCrypto.decrypt(
    ...
    if (
      !my_Type_Guard_function(decryptedSettings.level, [
        "easy",
        "medium",
        "hard",
      ]) || //-when not valid data
